...
**Windows PowerShell:**
```powershell
$env:ConnectionStrings__Default="Host=localhost;Port=5432;Database=businessweb;Username=postgres;Password=postgres"
$env:Jwt__Secret="PUT_LONG_RANDOM_SECRET_32+"
$env:Jwt__LifetimeMinutes="120"
```
...
Swagger: `http://localhost:5000/swagger` (portni `launchSettings.json` yoki console output’dan oling)
...
> `DatabaseSeeder` faqat Development’da auto-migrate bajaradi; Production’da migrationni qo‘lda bajaring.
...
## ✅ Minimal test flow (Swagger)

1) **Create Store**: `POST /api/stores`
2) **Create Product**: `POST /api/products`
3) **Create ProductPackage**: `POST /api/productpackages`
4) **Create StockIn**: `POST /api/stockins`
5) **Create Sale**: `POST /api/sales` (Cash/Debt/Partial)
6) **Pay Debt**: `POST /api/debts/pay`
7) **View Reports**: `GET /api/reports/daily`, `/monthly`, `/product/{id}`, `/stock`
...
4) Start command:
   ```bash
   dotnet out/BusinessWeb.API.dll
   ```
5) Deploy qiling. Production’da auto-migrate disabled (manual migration recommended).
